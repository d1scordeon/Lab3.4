...
public class ThreadCreator {
    ...
    public boolean createAndRunThreads(int threadQuantity) {
        ...
        for (int i = 1; i < threadQuantity; i++) {
            countingThreads.add(new CountingThreads(equations, equations.subList((equations.size() / 8) * i,
                    (equations.size() / 8) * (i + 1))));
        }
        try {
            ...
            System.out.println("Execution time with " + threadQuantity + " threads is: " + intervalThread.getSeconds()
                    + " seconds.");
